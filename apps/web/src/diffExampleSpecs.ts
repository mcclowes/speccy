...
const bookSchema = {
  ...
  properties: {
    id: { type: 'string' },
    title: { type: 'string' },
    availability: { type: 'string', enum: ['available', 'loaned'] },
  },
};
...
function library(
  version: string,
  overrides: (document: OpenAPIDocument) => void,
): OpenAPIDocument {
...
export const LIBRARY_V2 = library('2.0.0', (document) => {
  ...
  document.paths!['/books']!.get!.parameters = [
    {
      name: 'availability',
      in: 'query',
      schema: { type: 'string', enum: ['available', 'loaned'] },
    },
  ];
  ...
  document.paths!['/loans']!.get!.description =
    'Returns every loan, newest first, in the lending library’s local time zone.';
});
