...
describe('studio routing', () => {
  it('parses reference-scoped nested routes', () => {
    expect(
      parseStudioRoute({
        pathname: '/references/catalog/operations/list-companies',
        search: '',
      }),
    ).toEqual({
      page: 'reference',
  ...
  it('keeps a remote source locator in nested links', () => {
    const href = referenceHref(
      'catalog',
      { page: 'tag', tag: 'companies' },
      'https://example.com/openapi.yaml?version=2',
    );

    expect(href).toBe(
      '/references/catalog/tags/companies?source=https%3A%2F%2Fexample.com%2Fopenapi.yaml%3Fversion%3D2',
    );
