...
Large catalog specifications are processed in the browser. The studio keeps shared schema data in memory once, so highly connected documents such as Stripe remain responsive while they load. API health checks start in the background after the reference renders, prioritize the page you are viewing, and then index the rest of the document in bounded batches. The API health control shows quiet progress while that work continues.
