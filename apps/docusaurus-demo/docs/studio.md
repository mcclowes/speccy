...
Large catalog specifications are processed in the browser. The studio keeps local references compact and passes the parsed document directly to the renderer, so highly connected documents such as Stripe don't expand into a much larger intermediate copy. API health checks start in the background after the reference renders, prioritize the page you are viewing, and then index the rest of the document in bounded batches. The API health drawer shows the current phase, completed batches, and partial finding counts while that work continues.
