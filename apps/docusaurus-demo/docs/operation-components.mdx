...
In Docusaurus, use `SpeccyOperationReferenceProvider` in `src/theme/Root.tsx`. It reads the compact operation catalogs generated by every configured Speccy plugin instance, so the site does not bundle or parse complete OpenAPI documents on every documentation page:
