...
| Parameters in path, query, header, and cookie locations, including operation-level overrides of path parameters                   | Supported | `parameterSerialization.test.ts`, `OpenApi311.test.tsx`, `model.test.ts`                                                                                                                                                                                 |
| Parameter `style`, `explode`, `allowReserved`, `allowEmptyValue`, `deprecated`, `content`, examples, and defaults                 | Supported | `parameterSerialization.test.ts`, `OpenApi311.test.tsx`; `allowReserved` untested, tracked in [#35](https://github.com/mcclowes/speccy/issues/35)                                                                                                        |
...
| Responses, headers (schema or `content`), examples, links, runtime expressions, and link servers                                  | Supported | `OpenApi311.test.tsx`, `Speccy.test.tsx`                                                                                                                                                                                                                 |
...
| Local references, reference siblings, JSON Pointers, anchors, and dynamic anchors                                                 | Supported | `externalRefs.test.ts`, `model.test.ts`; `$anchor` and `$dynamicAnchor` resolve only through `resolveExternalRefs`, not the synchronous in-document resolver, and dynamic anchors are untested — see [#35](https://github.com/mcclowes/speccy/issues/35) |
...
| JSON Schema applicators, conditionals, dependent schemas, and unevaluated values                                                  | Supported | `OpenApi311.test.tsx` applicator test, `SchemaView.test.tsx` explorer fallback test                                                                                                                                                                      |
| Recursive `true` and `false` schemas in nested properties and array items                                                         | Supported | `SchemaView.test.tsx`, `diffSpecs.test.ts`                                                                                                                                                                                                               |
| OpenAPI XML metadata, discriminator property and mappings, read/write visibility, deprecation, and examples                       | Supported | `OpenApi311.test.tsx`, `SchemaView.test.tsx`, `model.test.ts`                                                                                                                                                                                            |
...
## Two schema presentations

Schemas reach the page through one of two renderers, and the matrix above
covers both. The schema explorer presents an object as a selectable field tree
and handles declared properties, `allOf`, root and field-level `oneOf` and
`anyOf`, `patternProperties`, and `additionalProperties`. Anything it cannot
express as a field row — `if`, `then`, `else`, `not`, `$defs`, `prefixItems`,
`contains`, `propertyNames`, `dependentSchemas`, `unevaluatedProperties`, and
`unevaluatedItems` — falls back to the recursive schema view, which renders
every keyword.

Adding a keyword to the explorer without adding it to that fallback list is how
a keyword goes missing from operation bodies while still rendering in the
reusable schemas section. Keep `unsupportedByExplorer` in
`packages/renderer/src/schemaExplorerModel.ts` aligned with what the field tree
actually renders.
