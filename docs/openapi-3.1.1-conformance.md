...
documentation rendering, request samples and execution, and lossless download.
...
cannot assign them custom presentation semantics.
...
| Parameter `style`, `explode`, `allowReserved`, `allowEmptyValue`, `content`, examples, and defaults                               | Supported | `parameterSerialization.test.ts`, `OpenApi311.test.tsx`                       |
...
| Components: schemas, responses, parameters, examples, request bodies, headers, security schemes, links, callbacks, and Path Items | Supported | `OpenApi311.test.tsx`, `ReferenceSections.test` coverage in `Speccy.test.tsx` |
| API key, HTTP, OAuth 2, OpenID Connect, and mutual-TLS security descriptions                                                      | Supported | `OpenApi311.test.tsx`, `SecurityScheme.test.tsx`, `Speccy.test.tsx`           |
| Security requirement alternatives and combined requirements                                                                       | Supported | `SecurityScheme.test.tsx`, `Speccy.test.tsx`                                  |
| Local references, reference siblings, JSON Pointers, anchors, and dynamic anchors                                                 | Supported | `externalRefs.test.ts`, `model.test.ts`                                       |
| Relative and absolute multi-document references with nested base URIs                                                             | Supported | `externalRefs.test.ts`, `fragmentedSpec.test.ts`, web app tests               |
...
| Recursive `true` and `false` schemas in every schema-bearing position                                                             | Supported | `SchemaView.test.tsx`, `diffSpecs.test.ts`                                    |
...
| Specification extensions and unknown JSON Schema vocabulary keywords                                                              | Preserved | Parser/model round-trip and download tests                                    |
