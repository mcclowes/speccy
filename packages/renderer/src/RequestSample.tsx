...
/** The request split into the shapes every generator draws on. */
...
interface LanguageDefinition {
  value: RequestSampleLanguage;
  label: string;
  icon: ReactNode;
  generate: (prepared: PreparedRequest) => string;
}
...
function prepareRequest(request: RequestSampleValue): PreparedRequest {
...
const LANGUAGES: LanguageDefinition[] = [
  ...
  },
  {
    ...
    generate: ({ request, options }) =>
      `const response = await fetch(${quote(request.url)}, ${JSON.stringify(options, null, 2)});\nconst data = await response.json();`,
  },
  {
    value: 'node',
    label: 'Node.js',
  ...
  },
  {
    ...
    generate: ({ request, headers }) =>
      `using var request = new HttpRequestMessage(HttpMethod.${request.method[0] + request.method.slice(1).toLowerCase()}, ${quote(request.url)});${headers.map(([name, value]) => `\nrequest.Headers.TryAddWithoutValidation(${quote(name)}, ${quote(value)});`).join('')}${request.body ? `\nrequest.Content = new StringContent(${quote(request.body)});` : ''}\nvar response = await new HttpClient().SendAsync(request);`,
  },
  {
    value: 'php',
    label: 'PHP',
  ...
  },
  {
    ...
    generate: ({ request, headers }) =>
      `req, err := http.NewRequest(${quote(request.method)}, ${quote(request.url)}, ${request.body ? `strings.NewReader(${quote(request.body)})` : 'nil'})\nif err != nil { log.Fatal(err) }${headers.map(([name, value]) => `\nreq.Header.Set(${quote(name)}, ${quote(value)})`).join('')}\nresponse, err := http.DefaultClient.Do(req)\nif err != nil { log.Fatal(err) }\ndefer response.Body.Close()`,
  },
];

/** Unknown languages fall back to the last entry (Go), matching the original chain's default arm. */
...
export function generateRequestSample(
  ...
  return languageDefinition(language).generate(prepareRequest(request));
}
...
export function RequestSample({
  ...
  const sample = selected.generate(prepareRequest(request));
  const copySample = selected.generate(prepareRequest(copyRequest));
