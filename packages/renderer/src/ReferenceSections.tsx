...
function Section({
  ...
  const [activeId, setActiveId] = useState(() =>
    names[0] ? componentAnchorId(activeKey, names[0]) : '',
  );
    ...
    >
      ...
      {names.length > 1 && (
        <label className={`sp-reference-jump ${styles.jump}`}>
          ...
          >
            {names.map((name) => (
              <option value={componentAnchorId(activeKey, name)} key={name}>
                {name}
              </option>
            ))}
          </select>
...
function operationLabel(value: unknown): string {
  ...
  return 'operationId' in value && typeof value.operationId === 'string'
    ? value.operationId
    : '';
}
  ...
  return (
    <>
      <div className="sp-schema-head">
        <span className="sp-type">{scheme.type ?? 'unknown'}</span>
        {scheme.scheme && <code>{scheme.scheme}</code>}
        {scheme.in && (
          <span>
      ...
      {scheme.bearerFormat && (
        <p>
          Bearer format: <code>{scheme.bearerFormat}</code>
        </p>
      )}
        ...
        entries(flows).map(([name, flow]) => (
          <div className={`sp-auth-flow ${styles.authFlow}`} key={name}>
            ...
            {flow.authorizationUrl && (
              <p>
                Authorization URL: <code>{flow.authorizationUrl}</code>
              </p>
            )}
            {flow.tokenUrl && (
              <p>
