...
export interface OasdiffAdapterOptions {
  ...
  operationMetadata?: (change: OasdiffChange) => { tag?: string; location?: string[] } | undefined;
}
...
function severityFor(level: string | number): DiffSeverity {
  ...
  if (normalized === 'err' || normalized === 'error' || normalized === '1') return 'breaking';
  if (normalized === 'warn' || normalized === 'warning' || normalized === '2') return 'warning';
...
function areaFor(change: OasdiffChange): DiffArea {
  const value = `${change.section ?? ''} ${change.id ?? ''}`.toLocaleLowerCase();
  if (/document|description|summary|external-doc/.test(value)) return 'documentation';
...
function locationFor(change: OasdiffChange, method?: HttpMethod): string[] {
  if (change.path && method) return ['paths', change.path, method, change.section].filter((value): value is string => Boolean(value));
  return [change.section, change.id].filter((value): value is string => Boolean(value));
}
 ...
 */
export function adaptOasdiffChangelog(input: unknown, options: OasdiffAdapterOptions = {}): DiffReport {
  if (!Array.isArray(input)) throw new TypeError('oasdiff changelog JSON must be an array.');
  const changes = input.map((value, index): ApiChange => {
    if (!value || typeof value !== 'object') throw new TypeError(`oasdiff change at index ${index} must be an object.`);
    const change = value as OasdiffChange;
    if (!change.id || !change.text) throw new TypeError(`oasdiff change at index ${index} must include id and text.`);
    ...
    return {
      ...
      severity: area === 'documentation' ? 'documentation' : severityFor(change.level),
      kind: kindFor(change),
      method,
      path: change.path,
      ...
      message: change.comment ? `${change.text} ${change.comment}` : change.text,
      source: {
  ...
  return { base: options.base ?? {}, revision: options.revision ?? {}, changes };
}
