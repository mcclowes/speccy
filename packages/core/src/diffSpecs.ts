...
import { HTTP_METHODS, normalizeDocument, operationsInDeclarationOrder, parseSpec } from './model';
import type { ApiChange, DiffArea, DiffKind, DiffOperation, DiffReport, DiffSeverity, DiffSpecVersion } from './diff';
import type { HttpMethod, OpenAPIDocument, Operation, Parameter, PathItem, ResponseObject, SchemaObject, SecurityRequirement } from './types';
...
function componentSchemas(document: OpenAPIDocument): Record<string, SchemaObject> {
...
function dereference(schema: SchemaObject | undefined, document: OpenAPIDocument, seen = new Set<string>()): SchemaObject | undefined {
...
function bodySchema(container: { content?: Record<string, { schema?: SchemaObject }>; schema?: SchemaObject } | undefined): SchemaObject | undefined {
  if (!container) return undefined;
  return container.content?.['application/json']?.schema ?? Object.values(container.content ?? {})[0]?.schema ?? container.schema;
}

function parametersFor(pathItem: PathItem | undefined, operation: Operation): Parameter[] {
...
function componentUsage(document: OpenAPIDocument): Map<string, ComponentUsage> {
  ...
  const collect = (node: unknown, into: Set<string>, seen: Set<unknown>) => {
    ...
    for (const [key, value] of Object.entries(node)) {
      ...
      const pointers = key === 'discriminator' && value && typeof value === 'object'
        ? Object.values((value as { mapping?: Record<string, string> }).mapping ?? {})
        : key === '$ref' && typeof value === 'string' ? [value] : [];
      let followed = false;
...
function compareSchemas(
  ...
  if (left.type && right.type && left.type !== right.type) {
    emit({
      ruleId: 'field-type-changed', severity: 'breaking', kind: 'changed', context, location, label,
      message: `${label} changed type from ${left.type} to ${right.type}.`, before: left.type, after: right.type,
    });
  } else if (left.format !== right.format && (left.format || right.format)) {
    emit({
      ruleId: 'field-format-changed', severity: 'breaking', kind: 'changed', context, location, label,
      message: `${label} changed format from ${left.format ?? 'none'} to ${right.format ?? 'none'}.`, before: left.format, after: right.format,
    });
  }
  ...
  if (left.enum && right.enum) {
    ...
    if (removed.length) emit(direction === 'request'
      ? { ruleId: 'request-enum-narrowed', severity: 'breaking', kind: 'changed', context, location, label, message: `${label} no longer accepts ${removed.join(', ')}.`, before: left.enum, after: right.enum }
      : { ruleId: 'response-enum-narrowed', severity: 'compatible', kind: 'changed', context, location, label, message: `${label} no longer returns ${removed.join(', ')}.`, before: left.enum, after: right.enum });
    if (added.length) emit(direction === 'request'
      ? { ruleId: 'request-enum-widened', severity: 'compatible', kind: 'changed', context, location, label, message: `${label} also accepts ${added.join(', ')}.`, before: left.enum, after: right.enum }
      : { ruleId: 'response-enum-widened', severity: 'warning', kind: 'changed', context, location, label, message: `${label} can now return ${added.join(', ')}, which existing clients have never seen.`, before: left.enum, after: right.enum });
  }
  ...
  for (const name of Object.keys(leftProperties)) {
    ...
    if (!(name in rightProperties)) {
      emit(direction === 'request'
        ? { ruleId: 'request-field-removed', severity: 'warning', kind: 'removed', context, location: fieldLocation, label: fieldLabel, message: `${fieldLabel} is no longer accepted.` }
        : { ruleId: 'response-field-removed', severity: 'breaking', kind: 'removed', context, location: fieldLocation, label: fieldLabel, message: `${fieldLabel} is no longer returned.` });
      continue;
    }
    if (direction === 'response' && leftRequired.has(name) && !rightRequired.has(name)) {
      emit({ ruleId: 'response-field-optional', severity: 'breaking', kind: 'changed', context, location: fieldLocation, label: fieldLabel, message: `${fieldLabel} is no longer guaranteed to be present.` });
    }
    if (direction === 'request' && !leftRequired.has(name) && rightRequired.has(name)) {
      emit({ ruleId: 'request-field-required', severity: 'breaking', kind: 'changed', context, location: fieldLocation, label: fieldLabel, message: `${fieldLabel} is now required.` });
    }
    compareSchemas(leftProperties[name], rightProperties[name], baseDocument, revisionDocument, direction, fieldLocation, fieldLabel, context, emit, seen);
  }
  ...
  compareSchemas(left.items, right.items, baseDocument, revisionDocument, direction, [...location, 'items'], `${label}[]`, context, emit, seen);
  ...
  for (const keyword of ['allOf', 'oneOf', 'anyOf'] as const) {
    ...
    if (leftMembers.length !== rightMembers.length) {
      emit({
        ruleId: 'schema-composition-changed', severity: 'warning', kind: 'changed', context, location: [...location, keyword], label,
    ...
    for (let index = 0; index < shared; index += 1) {
      compareSchemas(leftMembers[index], rightMembers[index], baseDocument, revisionDocument, direction, [...location, keyword, String(index)], label, context, emit, seen);
    }
  }
}
...
function compareOperation(
  ...
  const at = (area: DiffArea): Context => ({ method, path, operationId, tag, area });
  ...
  if (!baseOperation.deprecated && revisionOperation.deprecated) {
    emit({ ruleId: 'operation-deprecated', severity: 'warning', kind: 'deprecated', context: at('operation'), location, message: `${name} is now deprecated.` });
  }
  for (const field of ['summary', 'description'] as const) {
    if ((baseOperation[field] ?? '') !== (revisionOperation[field] ?? '')) {
      emit({
        ruleId: `operation-${field}-changed`, severity: 'documentation', kind: 'changed', context: at('documentation'),
        location: [...location, field], message: `The ${name} ${field} changed.`, before: baseOperation[field], after: revisionOperation[field],
      });
    }
  }

  const baseParameters = new Map(parametersFor(basePathItem, baseOperation).map((parameter) => [parameterKey(parameter), parameter]));
  const revisionParameters = new Map(parametersFor(revisionPathItem, revisionOperation).map((parameter) => [parameterKey(parameter), parameter]));
  ...
  for (const [key, parameter] of revisionParameters) {
    ...
    if (!previous) {
      if (parameter.required) {
        emit({ ruleId: 'required-parameter-added', severity: 'breaking', kind: 'added', context: at('parameters'), location: parameterLocation, label: parameter.name, message: `${name} now requires the ${parameter.name} ${parameter.in} parameter.` });
      } else {
        emit({ ruleId: 'parameter-added', severity: 'compatible', kind: 'added', context: at('parameters'), location: parameterLocation, label: parameter.name, message: `${name} accepts a new ${parameter.name} ${parameter.in} parameter.` });
      }
      continue;
    }
    if (!previous.required && parameter.required) {
      emit({ ruleId: 'required-parameter-added', severity: 'breaking', kind: 'changed', context: at('parameters'), location: parameterLocation, label: parameter.name, message: `${parameter.name} is now a required ${parameter.in} parameter of ${name}.` });
    }
    compareSchemas(previous.schema, parameter.schema, baseDocument, revisionDocument, 'request', [...parameterLocation, 'schema'], parameter.name ?? key, at('parameters'), emit, seen);
  }

  for (const [key, parameter] of baseParameters) {
    if (revisionParameters.has(key)) continue;
    emit({ ruleId: 'parameter-removed', severity: 'breaking', kind: 'removed', context: at('parameters'), location: [...location, 'parameters', key], label: parameter.name, message: `${name} no longer declares the ${parameter.name} ${parameter.in} parameter.` });
  }

  compareSchemas(
    bodySchema(baseOperation.requestBody), bodySchema(revisionOperation.requestBody), baseDocument, revisionDocument,
  ...
  for (const [code, response] of Object.entries(baseResponses)) {
    ...
    if (!next) {
      emit({ ruleId: 'response-removed', severity: 'breaking', kind: 'removed', context: at('response-body'), location: responseLocation, label: code, message: `${name} no longer documents a ${code} response.` });
      continue;
    }
    compareSchemas(bodySchema(response), bodySchema(next), baseDocument, revisionDocument, 'response', responseLocation, `Response ${code}`, at('response-body'), emit, seen);
    compareResponseHeaders(code, response, next, at('headers'), responseLocation, name, emit);
  }
...
function compareResponseHeaders(code: string, before: ResponseObject, after: ResponseObject, context: Context, location: string[], name: string, emit: Emit) {
  for (const header of Object.keys(before.headers ?? {})) {
    if (after.headers && header in after.headers) continue;
    emit({ ruleId: 'response-header-removed', severity: 'breaking', kind: 'removed', context, location: [...location, 'headers', header], label: header, message: `${name} no longer returns the ${header} header on ${code}.` });
  }
  for (const header of Object.keys(after.headers ?? {})) {
    if (before.headers && header in before.headers) continue;
    emit({ ruleId: 'response-header-added', severity: 'compatible', kind: 'added', context, location: [...location, 'headers', header], label: header, message: `${name} returns a new ${header} header on ${code}.` });
  }
}

function compareServers(base: OpenAPIDocument, revision: OpenAPIDocument, emit: Emit) {
  const context: Context = { area: 'operation' };
  const baseUrls = (base.servers ?? []).map((server) => server.url).filter((url): url is string => Boolean(url));
  const revisionUrls = new Set((revision.servers ?? []).map((server) => server.url));
  for (const url of baseUrls) {
    if (revisionUrls.has(url)) continue;
    emit({ ruleId: 'server-removed', severity: 'breaking', kind: 'removed', context, location: ['servers', url], label: url, message: `The ${url} server is gone.` });
  }
  const baseSet = new Set(baseUrls);
  for (const url of revisionUrls) {
    if (!url || baseSet.has(url)) continue;
    emit({ ruleId: 'server-added', severity: 'compatible', kind: 'added', context, location: ['servers', url], label: url, message: `${url} is a new server.` });
  }
}
...
function pairPaths(base: OpenAPIDocument, revision: OpenAPIDocument) {
  ...
  const pairs: Array<{ basePath?: string; revisionPath?: string; renamed?: boolean }> = [];
  ...
  for (const path of basePaths) {
    ...
    const match = revisionPaths.find((candidate) => !claimed.has(candidate) && !basePaths.includes(candidate) && pathTemplate(candidate) === template);
    if (match) {
      claimed.add(match);
  ...
  for (const path of revisionPaths) {
    if (!claimed.has(path) && !basePaths.includes(path)) pairs.push({ revisionPath: path });
  }
  return pairs;
}

export function diffSpecs(base: OpenAPIDocument | string, revision: OpenAPIDocument | string, options: DiffSpecsOptions = {}): DiffReport {
  ...
  const emit: Emit = ({ ruleId, severity, kind, message, location, context, label, before, after }) => {
    let id = `${ruleId}:${location.join('/')}`;
    for (let suffix = 2; ids.has(id); suffix += 1) id = `${ruleId}:${location.join('/')}#${suffix}`;
    ids.add(id);
    changes.push({
  ...
  for (const { basePath, revisionPath, renamed } of pairPaths(baseDocument, revisionDocument)) {
    const basePathItem = basePath ? baseDocument.paths?.[basePath] : undefined;
    const revisionPathItem = revisionPath ? revisionDocument.paths?.[revisionPath] : undefined;
    ...
    if (renamed && basePath && revisionPath) {
      ...
      emit({
        ruleId: 'path-parameter-renamed', severity: 'breaking', kind: 'changed', context, location: ['paths', revisionPath],
        message: `${basePath} became ${revisionPath}.`, before, after,
      });
    }

    if (basePathItem && !revisionPathItem) {
      for (const [method, operation] of operationsInDeclarationOrder(basePathItem)) {
        emit({
          ruleId: 'operation-removed', severity: 'breaking', kind: 'removed', location: ['paths', basePath!, method],
          context: { method, path: basePath, operationId: operation.operationId, tag: operation.tags?.[0], area: 'operation' },
    ...
    if (revisionPathItem && !basePathItem) {
      for (const [method, operation] of operationsInDeclarationOrder(revisionPathItem)) {
        emit({
          ruleId: 'operation-added', severity: 'compatible', kind: 'added', location: ['paths', revisionPath!, method],
          context: { method, path: revisionPath, operationId: operation.operationId, tag: operation.tags?.[0], area: 'operation' },
    ...
    for (const method of HTTP_METHODS) {
      ...
      if (baseOperation && !revisionOperation) {
        emit({
          ruleId: 'operation-removed', severity: 'breaking', kind: 'removed', location: ['paths', basePath!, method],
          context: { method, path: basePath, operationId: baseOperation.operationId, tag: baseOperation.tags?.[0], area: 'operation' },
          message: `${describeOperation(method, basePath!)} was removed.`,
        });
      } else if (revisionOperation && !baseOperation) {
        emit({
          ruleId: 'operation-added', severity: 'compatible', kind: 'added', location: ['paths', revisionPath!, method],
          context: { method, path: revisionPath, operationId: revisionOperation.operationId, tag: revisionOperation.tags?.[0], area: 'operation' },
          message: `${describeOperation(method, revisionPath!)} is new.`,
        });
      } else if (baseOperation && revisionOperation) {
        compareOperation(method, revisionPath!, basePathItem, revisionPathItem, baseOperation, revisionOperation, baseDocument, revisionDocument, emit, seen);
      }
    }
  }
