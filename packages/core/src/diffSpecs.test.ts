...
function ruleIds(report: DiffReport): string[] {
  return report.changes.map((change) =>
    change.id.slice(0, change.id.indexOf(':')),
  );
}
...
describe('diffSpecs', () => {
  ...
  it('carries title and version for both sides', () => {
    ...
    expect(report.revision).toMatchObject({
      title: 'Lending',
      version: '2.0.0',
    });
  });
  ...
  it('accepts YAML strings as well as parsed documents', () => {
    ...
    expect(find(diffSpecs(base, revision), 'operation-removed')).toHaveLength(
      1,
    );
  });
  ...
  describe('operations', () => {
    ...
    it('treats an added operation as compatible', () => {
      ...
      expect(find(report, 'operation-added')[0]).toMatchObject({
        severity: 'compatible',
        kind: 'added',
      });
    });
    ...
    it('reports a renamed path parameter once rather than as a removal and an addition', () => {
      ...
      expect(find(report, 'path-parameter-renamed')[0]).toMatchObject({
        severity: 'breaking',
      });
    ...
    it('flags a newly deprecated operation', () => {
      ...
      const revision = document({
        '/loans': { get: operation({ deprecated: true }) },
      });

      expect(
        find(diffSpecs(base, revision), 'operation-deprecated')[0],
      ).toMatchObject({ kind: 'deprecated', severity: 'warning' });
    });

    it('classifies a changed description as documentation', () => {
      const base = document({
        '/loans': { get: operation({ description: 'Lists loans.' }) },
      });
      const revision = document({
        '/loans': { get: operation({ description: 'Lists every loan.' }) },
      });

      expect(
        find(diffSpecs(base, revision), 'operation-description-changed')[0],
      ).toMatchObject({ severity: 'documentation' });
    });
  });
  ...
  describe('parameters', () => {
    it('flags a newly required parameter as breaking', () => {
      const base = document({
        '/loans': {
          get: operation({ parameters: [{ name: 'status', in: 'query' }] }),
        },
      });
      const revision = document({
        '/loans': {
      ...
      expect(
        find(diffSpecs(base, revision), 'required-parameter-added')[0],
      ).toMatchObject({ severity: 'breaking' });
    });
    ...
    it('flags a required parameter that did not exist before', () => {
      ...
      expect(
        find(diffSpecs(base, revision), 'required-parameter-added'),
      ).toHaveLength(1);
    });
    ...
    it('treats a removed parameter as breaking', () => {
      ...
      expect(
        find(diffSpecs(base, revision), 'parameter-removed')[0],
      ).toMatchObject({ severity: 'breaking' });
    });
  ...
  describe('request bodies', () => {
    it('flags a newly required request body as breaking', () => {
      ...
      expect(
        find(diffSpecs(base, revision), 'request-body-required')[0],
      ).toMatchObject({ severity: 'breaking' });
    });
    ...
    it('flags a newly required request field as breaking', () => {
      ...
      expect(
        find(diffSpecs(base, revision), 'request-field-required')[0],
      ).toMatchObject({ severity: 'breaking' });
    });
    ...
    it('treats a new optional request field as compatible', () => {
      ...
      expect(
        find(diffSpecs(base, revision), 'request-field-added')[0],
      ).toMatchObject({ severity: 'compatible' });
    });
    ...
    it('flags a narrowed request enum as breaking and a widened one as compatible', () => {
      ...
      expect(
        find(
          diffSpecs(withValues(['draft', 'open']), withValues(['draft'])),
          'request-enum-narrowed',
        )[0],
      ).toMatchObject({ severity: 'breaking' });
      expect(
        find(
          diffSpecs(withValues(['draft']), withValues(['draft', 'open'])),
  ...
  describe('responses', () => {
    it('flags a removed response as breaking and an added one as compatible', () => {
      ...
      expect(find(report, 'response-removed')[0]).toMatchObject({
        severity: 'breaking',
      });
      expect(find(report, 'response-added')[0]).toMatchObject({
        severity: 'compatible',
      });
    });
    ...
    it('flags a removed response field as breaking', () => {
      ...
      expect(
        find(diffSpecs(base, revision), 'response-field-removed')[0],
      ).toMatchObject({ severity: 'breaking' });
    });
    ...
    it('treats a new response field as compatible', () => {
      ...
      expect(
        find(diffSpecs(base, revision), 'response-field-added')[0],
      ).toMatchObject({ severity: 'compatible', after: { type: 'number' } });
    });

    it('flags a response field that is no longer guaranteed', () => {
      const schema = (required: string[]): SchemaObject => ({
        type: 'object',
        required,
        properties: { id: { type: 'string' } },
      });
      const base = document({
        '/loans': {
          get: operation({
            responses: {
              '200': { description: 'OK', ...jsonBody(schema(['id'])) },
            },
          }),
        },
      });
      const revision = document({
        '/loans': {
          get: operation({
            responses: {
              '200': { description: 'OK', ...jsonBody(schema([])) },
            },
          }),
        },
      });
      ...
      expect(
        find(diffSpecs(base, revision), 'response-field-optional')[0],
      ).toMatchObject({ severity: 'breaking' });
    });
    ...
    it('warns when a response enum gains a value clients have not seen', () => {
      ...
      expect(
        find(
          diffSpecs(withValues(['open']), withValues(['open', 'settled'])),
    ...
    it('flags a changed field type as breaking', () => {
      ...
      expect(
        find(
          diffSpecs(withType('integer'), withType('string')),
          'field-type-changed',
        )[0],
      ).toMatchObject({ severity: 'breaking' });
    });
  ...
  describe('security', () => {
    it('flags a stricter requirement as breaking', () => {
      ...
      const revision = document({
        '/loans': { get: operation({ security: [{ oauth: ['loans:read'] }] }) },
      });

      expect(
        find(diffSpecs(base, revision), 'security-tightened')[0],
      ).toMatchObject({ severity: 'breaking' });
    });
    ...
    it('treats a relaxed requirement as compatible', () => {
      ...
      const revision = document({
        '/loans': { get: operation({ security: [] }) },
      });

      expect(
        find(diffSpecs(base, revision), 'security-relaxed')[0],
      ).toMatchObject({ severity: 'compatible' });
    });
    ...
    it('falls back to the document requirement when an operation declares none', () => {
      ...
      expect(
        find(diffSpecs(base, revision), 'security-tightened'),
      ).toHaveLength(1);
    });

    it('treats an added authentication alternative as relaxed', () => {
      const base = document({
        '/loans': { get: operation({ security: [{ oauth: [] }] }) },
      });
      const revision = document({
        '/loans': {
          get: operation({ security: [{ oauth: [] }, { apiKey: [] }] }),
        },
      });
      const report = diffSpecs(base, revision);
    ...
    it('flags an added requirement within one alternative as tightened', () => {
      ...
      const revision = document({
        '/loans': { get: operation({ security: [{ oauth: [], apiKey: [] }] }) },
      });
  ...
  describe('servers', () => {
    it('flags a removed server as breaking and an added one as compatible', () => {
      ...
      expect(find(report, 'server-removed')[0]).toMatchObject({
        severity: 'breaking',
      });
      expect(find(report, 'server-added')[0]).toMatchObject({
        severity: 'compatible',
      });
    });
  });
  ...
  describe('shared components', () => {
    const withErrorSchema = (properties: Record<string, SchemaObject>) =>
      document(
        {
          '/loans': {
            get: operation({
    ...
    it('counts a shared schema change once and lists the operations it reaches', () => {
      ...
      expect(
        changes[0]?.affectedOperations?.map((item) => item.operationId).sort(),
      ).toEqual(['listLoans', 'listPayments']);
    });
    ...
    it('uses request compatibility rules for a request-only component', () => {
      const withInput = (schema: SchemaObject) =>
        document(
          {
            '/loans': {
              post: operation({
                requestBody: jsonBody({ $ref: '#/components/schemas/Input' }),
              }),
            },
          },
      ...
      expect(find(report, 'request-field-required')[0]).toMatchObject({
        severity: 'breaking',
      });
          ...
          '/loans': {
            get: operation({
              responses: {
                '200': {
                  description: 'OK',
                  ...jsonBody({ $ref: '#/components/schemas/Loan' }),
                },
              },
            }),
          },
  ...
  describe('change identity', () => {
    it('keeps ids stable when unrelated operations are added around a change', () => {
      ...
      const [first] = find(
        diffSpecs(base, revision),
        'required-parameter-added',
      );
      const [second] = find(
        diffSpecs(base, padded),
        'required-parameter-added',
      );
    ...
    it('gives every change a distinct id', () => {
      const base = document({
        '/loans': { get: operation() },
        '/payments': { get: operation({ operationId: 'listPayments' }) },
      });
      const report = diffSpecs(base, document({}));

      expect(new Set(report.changes.map((change) => change.id)).size).toBe(
  ...
  describe('a real document', () => {
    ...
    const clone = (input: OpenAPIDocument) =>
      JSON.parse(JSON.stringify(input)) as OpenAPIDocument;
    ...
    it('reports a newly required parameter and an added operation', () => {
      ...
      revision.paths!['/managed-cards']!.post!.parameters = [
        { name: 'tenant', in: 'query', required: true },
      ];
      revision.paths!['/managed-cards']!.get = {
        operationId: 'listManagedCards',
        responses: { '200': { description: 'Cards' } },
      };

      expect(ruleIds(diffSpecs(managedCards, revision))).toEqual(
        expect.arrayContaining(['required-parameter-added', 'operation-added']),
      );
    });
    ...
    it('counts a nested shared component change once and attributes it to the reaching operation', () => {
      ...
      expect(changes).toHaveLength(1);
      expect(
        changes[0]?.affectedOperations?.map((item) => item.operationId),
      ).toEqual(['createManagedCard']);
    });
