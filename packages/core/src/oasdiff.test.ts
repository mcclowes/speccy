...
describe('adaptOasdiffChangelog', () => {
  ...
  it('maps documentation changes and rejects unknown levels', () => {
    expect(adaptOasdiffChangelog([{ id: 'api-description-updated', text: 'description changed', level: 'INFO', section: 'description' }]).changes[0])
      .toMatchObject({ severity: 'documentation', scope: { area: 'documentation' } });
    expect(() => adaptOasdiffChangelog([{ id: 'x', text: 'x', level: 'FATAL' }])).toThrow('Unsupported oasdiff level');
  });
});
