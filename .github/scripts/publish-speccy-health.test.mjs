...
import { addedLines, commentFor, diagnosticLine } from './publish-speccy-health.mjs';
...
test('maps an OpenAPI path to its YAML line', () => {
  ...
  assert.equal(diagnosticLine(document, lineCounter, ['components', 'schemas', 'Card', 'properties', 'status']), 6);
});

test('groups health findings into one marked review comment', () => {
  const body = commentFor('openapi.yaml', 5, [
    { ruleId: 'enum-values-documented', severity: 'suggestion', message: 'Values need descriptions.' },
    { ruleId: 'property-description', severity: 'suggestion', message: 'status has no description.', suggestion: 'Describe it.' },
  ]);
