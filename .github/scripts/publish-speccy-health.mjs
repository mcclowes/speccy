...
export function commentFor(path, line, diagnostics) {
  ...
  for (const diagnostic of sorted) {
    lines.push(
      `- \`${diagnostic.ruleId}\`: ${diagnostic.message}${diagnostic.suggestion ? ` ${diagnostic.suggestion}` : ''}`,
    );
  }
  return lines.join('\n');
}
...
async function main() {
  ...
  if (
    ...
    throw new Error(
      'GITHUB_REPOSITORY, PR_NUMBER, COMMIT_SHA, SPEC_PATH, REPORT_PATH, and GH_TOKEN are required.',
    );
  }
  ...
  for (const diagnostic of diagnostics) {
    const line =
      diagnostic.range?.start?.line !== undefined
  ...
  const expectedMarkers = new Set(
    [...byLine.keys()].map((line) => `${markerPrefix}${specPath}:${line} -->`),
  );
  for (const comment of existingComments) {
    if (
      comment.body?.startsWith(markerPrefix) &&
      !expectedMarkers.has(comment.body.split('\n')[0])
    ) {
      await github(`/repos/${repository}/pulls/comments/${comment.id}`, {
        method: 'DELETE',
      });
    }
  }
  ...
  for (const [line, findings] of byLine) {
    ...
    if (existing) {
      if (existing.body !== body) {
        await github(`/repos/${repository}/pulls/comments/${existing.id}`, {
          method: 'PATCH',
